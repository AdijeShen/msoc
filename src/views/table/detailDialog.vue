  ...
  <el-dialog title="任务详情" :visible.sync="outerVisible">
    ...
    <el-dialog width="30%" title="提供输入" :visible.sync="inputVisible" append-to-body>
      ...
      <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="submitInput" :loading="loading">确 定</el-button>
        </span>
    </el-dialog>
    <div>
      <el-form ref="func" :model="func" label-width="120px">
        ...
        <el-form-item label="输入提供方：" v-for="(m,index) in func.state" :key="index+'b'">
          <el-button :loading="loading" :type="m | buttonFilter" :disabled="isDisable(index)" @click="showinput(index)">{{func.who[index]+" : x"+(index+1)}}</el-button>
        </el-form-item>
        <el-form-item label="计算结果(密文)：">
          <div>{{encResult | resultFilter}}</div>
          <el-button type='primary' @click="decryption(functionId)" v-if="stateAllSatisfied()">解密 </el-button>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    decryption(functionId) {
      ...
      this.$store.dispatch('user/getresult', data).then((res) => {
        showSuccess("解密结果:" + res.data / this.func.divisor)
      }).catch(() => {})
    },
    loadFunction(functionId) {
      this.functionId = functionId
      this.$store.dispatch('user/loadfunctiondetail', functionId).then((res) => {
        ...
        if (this.stateAllSatisfied()) {
          ...
          this.$store.dispatch('user/getciphertext', data).then((res) => {
            let data = JSON.parse(res.data)
    ...
    showinput(index) {
      ...
      if (this.func.state[index] == 0) {
        ...
        this.$store.dispatch('user/getuserciphertext', data).then((res) => {
          let data = JSON.parse(res.data)
        ...
        if (this.$store.state.user.username == "CSP") {
          let data = {
            functionid: this.functionId
          }
          this.$store.dispatch('user/getcspciphertext', data).then((res) => {
            let data = JSON.parse(res.data)
    ...
    isDisable(index) {
      if (this.func.state[index] == 0 && this.func.who[index] != this.$store.state.user.username) {
        return true
      }
      return false
    },
    submitInput() {
      ...
      this.loading = true
      this.$store.dispatch('user/updatefunctionparams', data).then((res) => {
        showSuccess("提交成功")
